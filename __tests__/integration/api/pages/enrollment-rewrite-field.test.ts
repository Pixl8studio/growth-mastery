/**
 * Enrollment Page Rewrite Field API Integration Tests
 * Tests generating 3 AI-powered rewrite options for enrollment page fields
 */

import { describe, it, expect, vi, beforeEach } from "vitest";
import { POST } from "@/app/api/pages/enrollment/[pageId]/rewrite-field/route";
...
vi.mock("@/lib/ai/client", () => ({
    generateTextWithAI: vi.fn(async () => "Rewritten enrollment content variation"),
}));

describe("POST /api/pages/enrollment/[pageId]/rewrite-field", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    it("should generate 3 rewrite options successfully", async () => {
        const request = new NextRequest(
            "http://localhost:3000/api/pages/enrollment/test-page-id/rewrite-field",
            {
                method: "POST",
                body: JSON.stringify({
                    fieldContent: "Enroll in our program today",
                    fieldType: "heading",
                }),
            }
        );
    ...
    it("should handle different field types", async () => {
        const fieldTypes = ["heading", "subheading", "body", "cta"];
