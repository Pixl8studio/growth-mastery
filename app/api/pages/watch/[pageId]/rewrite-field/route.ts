/**
 * API Route: Rewrite Field for Watch Page
...
export async function POST(
    ...
    try {
        ...
        const optionsPromises = fieldStyles.map(async (style, index) => {
            const prompt = `Rewrite the following ${fieldType} for a video watch/webinar page in a ${style.toLowerCase()} style.

Original content: "${fieldContent}"

Requirements:
- Keep the core message but adjust the tone to match the style
- Make it engaging and value-focused
...
- Use active voice
- Create anticipation for video content
...
Provide ONLY the rewritten content, no explanations or quotes.`;
        ...
        Sentry.captureException(error, {
            tags: {
                component: "api",
                endpoint: "POST /api/pages/watch/[pageId]/rewrite-field",
            },
            extra: { pageId },
        });
