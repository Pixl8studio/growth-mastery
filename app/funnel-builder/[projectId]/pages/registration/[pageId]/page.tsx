 ...
 * Renders registration pages with optional visual editor (requires ?edit=true)
 */
...
export default async function RegistrationPageEditor({
    params,
    searchParams,
}: PageProps) {
    ...
    if (!user || userError) {
        logger.warn({ pageId }, "Unauthenticated access to page");
        redirect("/login");
    }
    ...
    if (pageError || !page) {
        ...
        redirect(`/funnel-builder/${projectId}/step/4`);
    }
    ...
    if (isEditMode && page.user_id !== user.id) {
        logger.warn(
            ...
            "User attempted to edit page they don't own"
        );
        // Redirect to view-only mode
        redirect(`/funnel-builder/${projectId}/pages/registration/${pageId}`);
    }
    ...
    if (!page.html_content) {
        logger.error({ pageId }, "Page missing HTML content");
        return (
            <div className="flex min-h-screen items-center justify-center">
                <div className="rounded-lg border border-red-200 bg-red-50 p-8 text-center">
                    ...
                    <a
                        href={`/funnel-builder/${projectId}/step/4`}
