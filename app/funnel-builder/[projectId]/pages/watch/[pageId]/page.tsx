/**
 * Watch Page Editor
 * Renders watch pages with optional visual editor (requires ?edit=true)
 */
...
export default async function WatchPageEditor({ params, searchParams }: PageProps) {
    ...
    if (!user || userError) {
        logger.warn({ pageId }, "Unauthenticated access to watch page");
        redirect("/login");
    }
    ...
    if (pageError || !page) {
        logger.error({ error: pageError, pageId }, "Watch page not found");
        redirect(`/funnel-builder/${projectId}/step/8`);
    }
    ...
    if (isEditMode && page.user_id !== user.id) {
        logger.warn(
            ...
            "User attempted to edit watch page they don't own"
        );
    ...
    if (!page.html_content) {
        logger.error({ pageId }, "Watch page missing HTML content");
        return (
            <div className="flex min-h-screen items-center justify-center">
                <div className="rounded-lg border border-red-200 bg-red-50 p-8 text-center">
                    ...
                    <a
                        href={`/funnel-builder/${projectId}/step/8`}
