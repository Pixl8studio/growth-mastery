 ...
 *
 * Connect and manage Facebook Page integration.
 */
...
export function FacebookIntegration({ projectId }: FacebookIntegrationProps) {
    ...
    const loadConnection = async () => {
        ...
        try {
            ...
            logger.error({ error, projectId }, "Failed to load Facebook connection");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleConnect = async () => {
        setConnecting(true);
        try {
            const redirectUri = `${window.location.origin}/api/funnel/${projectId}/integrations/facebook/callback`;
            const response = await fetch(
                `/api/funnel/${projectId}/integrations/facebook/connect?redirect_uri=${encodeURIComponent(redirectUri)}`
            );
    ...
    const handleDisconnect = async () => {
        try {
            ...
            logger.error({ error, projectId }, "Failed to disconnect Facebook");
            toast({
                title: "Error",
