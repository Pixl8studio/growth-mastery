/**
 * Icon Mapper for Vanilla JS
 * Provides SVG icons for the visual editor with interactive picker
 */
...
const ICON_SVG_MAP = {
    ...
    gem: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="6 3 18 3 22 9 12 22 2 9 6 3"/><path d="m12 22-4-13-2-6"/><path d="m12 22 4-13 2-6"/><path d="M2 9h20"/></svg>',
...
// Icon categories for organized picker
const ICON_CATEGORIES = {
    achievement: {
        name: 'Achievement & Success',
        icons: ['target', 'trophy', 'award', 'trending-up', 'star', 'sparkles']
    },
    action: {
        name: 'Action & Speed',
        icons: ['zap', 'rocket', 'flame']
    },
    learning: {
        name: 'Ideas & Learning',
        icons: ['lightbulb', 'brain', 'graduation-cap', 'book-open']
    },
    time: {
        name: 'Time & Progress',
        icons: ['clock', 'timer', 'hourglass']
    },
    money: {
        name: 'Money & Value',
        icons: ['dollar-sign', 'piggy-bank', 'gem', 'credit-card']
    },
    people: {
        name: 'People & Community',
        icons: ['users', 'user', 'heart', 'handshake']
    },
    communication: {
        name: 'Communication',
        icons: ['message-circle', 'mail', 'phone']
    },
    tools: {
        name: 'Tools & Settings',
        icons: ['settings', 'wrench', 'sliders']
    },
    security: {
        name: 'Security & Trust',
        icons: ['lock', 'shield', 'shield-check', 'check-circle', 'check']
    },
    other: {
        name: 'Other',
        icons: ['compass', 'map', 'gift']
    }
};

/**
 * Get SVG icon by name
 * @param {string} iconName - Name of the icon (e.g., 'target', 'zap')
 * @returns {string} SVG string
 */
function getIconSvg(iconName) {
    return ICON_SVG_MAP[iconName] || ICON_SVG_MAP['target'];
}
...
function getAllIconNames() {
    return Object.keys(ICON_SVG_MAP);
}

/**
 * Get icon display name (convert kebab-case to Title Case)
...
// Make available globally for visual editor
if (typeof window !== 'undefined') {
    window.getIconSvg = getIconSvg;
    window.getAllIconNames = getAllIconNames;
    window.getIconDisplayName = getIconDisplayName;
    window.ICON_SVG_MAP = ICON_SVG_MAP;
