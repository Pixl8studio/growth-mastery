...
class VisualEditor {
  constructor() {
    ...
    this.isEditMode = localStorage.getItem('visualEditor_editMode') === 'false' ? false : true;
  ...
  makeBlockEditable(block) {
    ...
    // Add section controls (arrows and X button)
    if (!block.querySelector('.section-controls')) {
      const controls = document.createElement('div');
      controls.className = 'section-controls';
      controls.innerHTML = `
        <button class="section-control-btn move-up-btn" title="Move Section Up" data-action="move-up">
          <span>↑</span>
        </button>
        <button class="section-control-btn move-down-btn" title="Move Section Down" data-action="move-down">
          <span>↓</span>
        </button>
        <button class="section-control-btn delete-section-btn" title="Delete Section" data-action="delete">
          <span>×</span>
        </button>
      `;
      ...
      controls.addEventListener('click', (e) => {
        ...
        if (action) {
          this.handleSectionAction(block, action);
        }
      });
    }
    ...
    const cardElements = block.querySelectorAll('.feature-card, .testimonial-card, .learn-card, .pricing-card, .story-card, .faq-item');
  ...
  getBlockSettings(blockType, element) {
    ...
    let commonSettings = {
      sectionBackground: { 
        ...
        options: [
          ...
          { value: 'linear-gradient(135deg, #1e3c72, #2a5298)', label: '🌌 Galaxy', preview: 'linear-gradient(135deg, #1e3c72, #2a5298)' },
          { value: 'linear-gradient(135deg, #ff6b6b, #feca57)', label: '🪸 Coral', preview: 'linear-gradient(135deg, #ff6b6b, #feca57)' },
          { value: 'linear-gradient(135deg, #48c9b0, #96e6a1)', label: '🌿 Mint', preview: 'linear-gradient(135deg, #48c9b0, #96e6a1)' },
          { value: 'linear-gradient(135deg, #a8edea, #fed6e3)', label: '💜 Lavender', preview: 'linear-gradient(135deg, #a8edea, #fed6e3)' },
          { value: 'linear-gradient(135deg, #ffecd2, #fcb69f)', label: '🍑 Peach', preview: 'linear-gradient(135deg, #ffecd2, #fcb69f)' },
          { value: 'linear-gradient(135deg, #74b9ff, #0984e3)', label: '🧊 Arctic', preview: 'linear-gradient(135deg, #74b9ff, #0984e3)' },
          { value: 'linear-gradient(135deg, #11998e, #38ef7d)', label: '💎 Emerald', preview: 'linear-gradient(135deg, #11998e, #38ef7d)' },
          { value: 'linear-gradient(135deg, #ee5a24, #feca57)', label: '🔥 Flame', preview: 'linear-gradient(135deg, #ee5a24, #feca57)' },
          { value: 'linear-gradient(135deg, #f093fb, #f5576c)', label: '🌸 Dawn', preview: 'linear-gradient(135deg, #f093fb, #f5576c)' },
      ...
      customTheme: {
        ...
        variables: [
          { name: '--text-primary', label: 'Primary Text', default: '#1F2937' },
          { name: '--text-secondary', label: 'Secondary Text', default: '#6B7280' },
          { name: '--bg-primary', label: 'Primary Background', default: '#FFFFFF' },
          { name: '--bg-secondary', label: 'Secondary Background', default: '#F9FAFB' },
          { name: '--primary-color', label: 'Primary Color', default: '#4F46E5' },
          { name: '--secondary-color', label: 'Secondary Color', default: '#10B981' }
        ]
      },
      paddingTop: { type: 'range', label: 'Top Padding', min: 0, max: 200, unit: 'px', default: 80 },
      paddingBottom: { type: 'range', label: 'Bottom Padding', min: 0, max: 200, unit: 'px', default: 80 },
      marginTop: { type: 'range', label: 'Top Margin', min: 0, max: 100, unit: 'px', default: 0 },
      marginBottom: { type: 'range', label: 'Bottom Margin', min: 0, max: 100, unit: 'px', default: 0 },
      borderRadius: { type: 'range', label: 'Border Radius', min: 0, max: 50, unit: 'px', default: 0 }
    };
    ...
    const typeSpecific = {
      hero: {
        ...commonSettings,
        titleSize: { type: 'range', label: 'Title Size', min: 24, max: 80, unit: 'px', default: 48 },
        subtitleSize: { type: 'range', label: 'Subtitle Size', min: 16, max: 32, unit: 'px', default: 20 },
        ...
        backgroundGradient: { type: 'checkbox', label: 'Gradient Background', default: false }
      },
      features: {
        ...commonSettings,
        cardsPerRow: { type: 'range', label: '📐 Cards Per Row', min: 1, max: 5, step: 1, default: 3 },
        columns: { type: 'range', label: 'Columns', min: 1, max: 4, default: 3 },
        cardShadow: { type: 'checkbox', label: 'Card Shadow', default: true },
        cardPadding: { type: 'range', label: 'Card Padding', min: 10, max: 60, unit: 'px', default: 32 },
        cardRadius: { type: 'range', label: 'Card Radius', min: 0, max: 30, unit: 'px', default: 16 }
      },
      testimonial: {
        ...commonSettings,
        cardsPerRow: { type: 'range', label: '📐 Cards Per Row', min: 1, max: 5, step: 1, default: 3 },
        layout: { type: 'select', label: 'Layout', options: ['grid', 'carousel', 'single'], default: 'grid' },
      ...
      learn: {
        ...commonSettings,
        cardsPerRow: { type: 'range', label: '📐 Cards Per Row', min: 1, max: 4, step: 1, default: 2 },
        layout: { type: 'select', label: 'Layout', options: ['grid', 'single'], default: 'grid' },
        showNumbers: { type: 'checkbox', label: 'Show Numbers', default: true },
        cardRadius: { type: 'range', label: 'Card Radius', min: 0, max: 30, unit: 'px', default: 16 },
        cardShadow: { type: 'checkbox', label: 'Card Shadow', default: true },
        titleColor: { type: 'color', label: 'Title Color', default: '#333333' },
        descriptionColor: { type: 'color', label: 'Description Color', default: '#666666' },
        numberColor: { type: 'color', label: 'Number Color', default: '#4f46e5' }
      },
      pricing: {
        ...commonSettings,
        cardsPerRow: { type: 'range', label: '📐 Cards Per Row', min: 1, max: 5, step: 1, default: 3 },
        ...
        cardShadow: { type: 'checkbox', label: 'Card Shadow', default: true },
        cardRadius: { type: 'range', label: 'Card Radius', min: 0, max: 30, unit: 'px', default: 12 },
        showBadge: { type: 'checkbox', label: 'Show Popular Badge', default: true }
      },
      'webinar-cta': {
        ...commonSettings,
        overlayOpacity: { type: 'range', label: 'Overlay Opacity', min: 0, max: 100, unit: '%', default: 90 },
        buttonStyle: { type: 'select', label: 'Button Style', options: ['primary', 'secondary', 'outline'], default: 'primary' },
        textAlign: { type: 'select', label: 'Text Alignment', options: ['left', 'center', 'right'], default: 'center' }
      },
      'social-proof': {
        ...commonSettings,
        cardsPerRow: { type: 'range', label: '📐 Cards Per Row', min: 1, max: 5, step: 1, default: 3 },
        cardSpacing: { type: 'range', label: '📏 Card Spacing', min: 0.5, max: 15.5, step: 0.5, unit: 'rem', default: 2 },
        cardPadding: { type: 'range', label: '📦 Card Padding', min: 1, max: 4, step: 0.25, unit: 'rem', default: 2 },
        cardRadius: { type: 'range', label: '🎨 Card Roundness', min: 0, max: 30, unit: 'px', default: 12 },
        showShadows: { type: 'checkbox', label: '✨ Card Shadows', default: true },
        hideBorders: { type: 'checkbox', label: '🚫 Hide Borders', default: false },
        hideBackground: { type: 'checkbox', label: '👻 Hide Background', default: false }
      }
    };
  ...
  getTextSettings() {
    return {
      ...
      fontSize: { type: 'range', label: 'Font Size', min: 12, max: 72, unit: 'px', default: 16 },
      fontWeight: { type: 'select', label: 'Font Weight', options: ['300', '400', '500', '600', '700', '800'], default: '400' },
      lineHeight: { type: 'range', label: 'Line Height', min: 1, max: 3, step: 0.1, default: 1.5 },
      letterSpacing: { type: 'range', label: 'Letter Spacing', min: -2, max: 5, unit: 'px', default: 0 },
      textAlign: { type: 'select', label: 'Text Alignment', options: ['left', 'center', 'right', 'justify'], default: 'left' },
      textTransform: { type: 'select', label: 'Text Transform', options: ['none', 'uppercase', 'lowercase', 'capitalize'], default: 'none' }
    };
  }
  ...
  getCardSettings() {
    return {
      ...
      cardShadow: { type: 'checkbox', label: 'Card Shadow', default: true },
      cardRadius: { type: 'range', label: 'Card Radius', min: 0, max: 30, unit: 'px', default: 16 },
      cardPadding: { type: 'range', label: 'Card Padding', min: 10, max: 60, unit: 'px', default: 24 },
      cardBorder: { type: 'checkbox', label: 'Card Border', default: false },
      cardBorderColor: { type: 'color', label: 'Border Color', default: '#E2E8F0' },
      customCardColor: {
  ...
  getButtonSettings() {
    return {
      ...
      buttonSize: { type: 'select', label: 'Button Size', options: ['small', 'medium', 'large'], default: 'medium' },
      buttonRadius: { type: 'range', label: 'Button Radius', min: 0, max: 50, unit: 'px', default: 8 },
      buttonPadding: { type: 'range', label: 'Button Padding', min: 5, max: 30, unit: 'px', default: 12 },
      buttonShadow: { type: 'checkbox', label: 'Button Shadow', default: true },
      buttonBorder: { type: 'checkbox', label: 'Button Border', default: false },
      buttonBorderColor: { type: 'color', label: 'Border Color', default: '#E2E8F0' },
      customButtonColor: {
  ...
  renderSettingsPanel(settings, element, panelTitle = null) {
    ...
    if (blockType === 'quote') {
      html += this.renderImageUploadSection('Background Image', 'background', element, 'Change the background image for this quote section');
    ...
    Object.entries(settings).forEach(([key, setting]) => {
      ...
      } else if (element.classList && (element.classList.contains('feature-card') || element.classList.contains('testimonial-card') || element.classList.contains('learn-card') || element.classList.contains('pricing-card') || element.classList.contains('story-card') || element.classList.contains('faq-item'))) {
      ...
      switch (setting.type) {
        case 'section-background-select':
          ...
          setting.options.forEach(option => {
            const isSelected = currentValue === option.value ? 'selected' : '';
            html += `
        ...
        case 'text-color-editor':
          ...
          const selectedTextColor = this.getTextProperty(element, 'textColor');
        ...
        case 'block-color-editor':
          ...
          const selectedBlockColor = this.getBlockProperty(element, 'blockColor');
        ...
        case 'card-color-editor':
          ...
          const selectedCardColor = this.getCardProperty(element, 'cardBackground');
        ...
        case 'custom-theme-editor':
          ...
          const selectedBg = this.getBlockProperty(element, 'sectionBackground');
        ...
        case 'checkbox':
          ...
          break;
        case 'text':
          const placeholder = setting.placeholder || `Enter ${setting.label.toLowerCase()}`;
  ...
  renderElementManagementSection(block) {
    ...
    switch (blockType) {
      ...
      case 'pricing':
        ...
        const pricingFeatureCount = block.querySelectorAll('.pricing-features li').length;
        managementHTML += `
          <div class="management-info">
            <div class="management-stats">
  ...
  handleDragStart(e) {
    ...
    console.log('Block has .block class:', this.draggedBlock.classList.contains('block'));
  }
      ...
      try {
        if (draggedIndex < dropIndex) {
          ...
          if (dropZone.nextSibling) {
            pageContainer.insertBefore(this.draggedBlock, dropZone.nextSibling);
  ...
  applyThemeColorsToBlock(block) {
    ...
    if (block.classList.contains('hero-block')) {
      ...
      const heroTexts = block.querySelectorAll('h1, h2, h3, h4, h5, h6, p, span, div, .hero-title, .hero-subtitle');
    ...
    if (block.classList.contains('cta-block')) {
      block.style.setProperty('background', gradientBg, 'important');
      const ctaTexts = block.querySelectorAll('h1, h2, h3, h4, h5, h6, p, span, div');
  ...
  undo() {
    if (this.undoStack.length > 1) {
      ...
      this.initializeBlocks();
      this.updateToolbar();
      console.log('Undo applied - Stack lengths:', this.undoStack.length, this.redoStack.length);
    }
  }
  ...
  redo() {
    if (this.redoStack.length > 0) {
      ...
      this.initializeBlocks();
      this.updateToolbar();
      console.log('Redo applied - Stack lengths:', this.undoStack.length, this.redoStack.length);
    }
  }
  ...
  setupEventListeners() {
    ...
    document.addEventListener('input', (e) => {
      if (e.target.dataset.property && this.selectedBlock) {
        ...
        if (this.selectedButtonElement) {
          ...
          this.updateCardProperty(this.selectedCardElement, e.target.dataset.property, value);
        } else if (this.selectedTextElement) {
          this.updateTextProperty(this.selectedTextElement, e.target.dataset.property, value);
        } else if (this.selectedBlock) {
          this.updateBlockProperty(this.selectedBlock, e.target.dataset.property, value);
        }
      }
    });
    ...
    document.addEventListener('input', (e) => {
      if (e.target.type === 'color' && e.target.dataset.property) {
        const textInput = document.querySelector(`input[data-property="${e.target.dataset.property}-text"]`);
        if (textInput) {
    ...
    document.addEventListener('click', (e) => {
      if (e.target && e.target.closest && e.target.closest('.theme-option') && (this.selectedBlock || this.selectedTextElement)) {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.ctrlKey || e.metaKey) {
        switch (e.key) {
          ...
          case 'Escape':
            e.preventDefault();
            this.closePanelSettings(document.getElementById('block-settings'));
            break;
        }
      }
    });
  ...
  getBlockProperty(block, property) {
    ...
    switch (property) {
      ...
      case 'titleColor':
        const title = block.querySelector('h1, h2, .hero-title, .heading-1, .heading-2');
      ...
      case 'subtitleColor':
        ...
        return subtitle ? this.rgbToHex(getComputedStyle(subtitle).color) : '#6b7280';
      case 'subtitleSize':
        const subtitleEl = block.querySelector('.hero-subtitle, .subheading');
        return subtitleEl ? parseInt(getComputedStyle(subtitleEl).fontSize) : 20;
      case 'cardsPerRow':
        ...
        const cardGrid = block.querySelector('.features-grid, .testimonial-grid, .learn-grid, .pricing-options, .social-proof-strip');
        if (cardGrid) {
  ...
  updateBlockProperty(block, property, value) {
    ...
    switch (property) {
      ...
      case 'buttonColor':
        ...
        buttons.forEach(btn => {
          if (btn.classList.contains('btn-primary')) {
            ...
            console.log(`Applied buttonColor: ${value} to primary button`, btn);
          } else if (btn.classList.contains('btn-secondary')) {
            btn.style.setProperty('border-color', value, 'important');
            btn.style.setProperty('color', value, 'important');
            console.log(`Applied buttonColor: ${value} to secondary button`, btn);
          }
        });
        break;
      ...
      case 'titleSize':
        ...
        break;
      case 'subtitleSize':
        const subtitleElements = block.querySelectorAll('.hero-subtitle, .subheading');
      ...
      case 'columns':
        ...
        if (grid) {
          grid.style.setProperty('grid-template-columns', `repeat(${value}, 1fr)`, 'important');
        }
        break;
      case 'cardsPerRow':
      ...
      case 'backgroundGradient':
        if (value) {
          block.style.setProperty('background', 'linear-gradient(135deg, rgba(79, 70, 229, 0.1), rgba(16, 185, 129, 0.1))', 'important');
        } else {
          block.style.removeProperty('background');
        }
        break;
      case 'cardShadow':
        const cardElements = block.querySelectorAll('.feature-card, .testimonial-card, .pricing-card');
        cardElements.forEach(card => {
          if (value) {
      ...
      case 'cardSpacing':
        ...
        block.style.setProperty('--social-proof-gap-mobile', Math.max(0.5, value - 0.5) + 'rem');
        break;
      case 'cardPadding':
        const socialCards = block.querySelectorAll('.social-proof-card');
        socialCards.forEach(card => {
          card.style.setProperty('padding', value + 'rem ' + (value * 0.75) + 'rem', 'important');
        });
        break;
      case 'cardRadius':
      ...
      case 'showShadows':
        ...
        socialCardElements.forEach(card => {
          if (value) {
            card.style.setProperty('box-shadow', '0 2px 8px rgba(0, 0, 0, 0.05)', 'important');
          } else {
            card.style.removeProperty('box-shadow');
          }
        });
        break;
      case 'hideBorders':
        const socialProofCardsBorder = block.querySelectorAll('.social-proof-card');
        socialProofCardsBorder.forEach(card => {
          if (value) {
            card.style.setProperty('border', 'none', 'important');
          } else {
            card.style.setProperty('border', '1px solid var(--border-light, #e2e8f0)', 'important');
          }
        });
        break;
      case 'hideBackground':
        const socialProofCardsBackground = block.querySelectorAll('.social-proof-card');
        socialProofCardsBackground.forEach(card => {
          if (value) {
            card.style.setProperty('background', 'transparent', 'important');
          } else {
            card.style.setProperty('background', 'var(--bg-primary, #ffffff)', 'important');
          }
        });
        break;
  ...
  updateTextProperty(textElement, property, value) {
    ...
    switch (property) {
      ...
      case 'lineHeight':
        ...
        break;
      case 'letterSpacing':
        textElement.style.setProperty('letter-spacing', value + 'px', 'important');
        break;
      case 'textAlign':
  ...
  updateCardProperty(cardElement, property, value) {
    ...
    switch (property) {
      case 'cardBackground':
        if (value === 'custom') {
          ...
          if (value.includes('linear-gradient') || value.includes('rgba')) {
            ...
            cardElement.style.setProperty('background-color', value, 'important');
          }
          cardElement.removeAttribute('data-custom-color');
        }
        break;
      case 'cardRadius':
        cardElement.style.setProperty('border-radius', value + 'px', 'important');
        break;
      case 'cardPadding':
      ...
      case 'cardShadow':
        if (value) {
          cardElement.style.setProperty('box-shadow', '0 4px 6px -1px rgba(0, 0, 0, 0.1)', 'important');
        } else {
      ...
      case 'cardBorder':
        if (value) {
          cardElement.style.setProperty('border', '1px solid #E2E8F0', 'important');
        } else {
    ...
    switch (property) {
      case 'buttonBackground':
        if (value === 'custom') {
          ...
          // Handle both solid colors and gradients
          if (value.includes('linear-gradient') || value.includes('rgba')) {
            buttonElement.style.setProperty('background', value, 'important');
          } else {
            buttonElement.style.setProperty('background-color', value, 'important');
          }
      ...
      case 'buttonTextColor':
        ...
        break;
      case 'buttonRadius':
        buttonElement.style.setProperty('border-radius', value + 'px', 'important');
        break;
      case 'buttonPadding':
      ...
      case 'buttonShadow':
        if (value) {
          buttonElement.style.setProperty('box-shadow', '0 4px 6px -1px rgba(0, 0, 0, 0.1)', 'important');
        } else {
      ...
      case 'buttonBorder':
        if (value) {
          buttonElement.style.setProperty('border', '1px solid #E2E8F0', 'important');
        } else {
  ...
  updateSettingsUI(property, value) {
    ...
    if (rangeInput && rangeInput.type === 'range') {
      ...
      if (nextSibling && nextSibling.classList.contains('range-value')) {
        const settings = this.getBlockSettings(this.selectedBlock?.dataset.blockType || 'generic');
  ...
  updateCustomVariable(variableName, value) {
    ...
    switch (variableName) {
      case '--text-primary':
        document.querySelectorAll('h1, h2, h3, .heading-1, .heading-2, .heading-3, p, .testimonial-quote').forEach(el => {
          el.style.setProperty('color', value, 'important');
        });
        break;
      case '--text-secondary':
        document.querySelectorAll('.subheading, .feature-description, .testimonial-info p').forEach(el => {
          el.style.setProperty('color', value, 'important');
        });
        break;
      case '--bg-section-1':
      ...
      case '--bg-section-3':
        ...
        break;
      case '--primary-color':
        document.querySelectorAll('.block-handle, .pricing-badge').forEach(el => {
          el.style.setProperty('background', value, 'important');
        });
        break;
  ...
  saveCustomTheme() {
    ...
    if (savedTheme) {
      try {
        ...
        this.showNotification(`Custom theme saved! (${variableCount} variables)`, 'success');
      } catch (e) {
  ...
  loadSavedCustomTheme() {
    ...
    if (savedTheme) {
      try {
        ...
        console.log('Loaded custom theme with', Object.keys(themeData).length, 'variables');
      } catch (e) {
  ...
  makePricingManageable(block) {
    ...
    pricingFeatures.forEach((feature, index) => {
      this.addElementControls(feature, 'pricing-feature', {
        onAdd: () => this.addPricingFeature(feature.closest('.pricing-card'), feature),
  ...
  addElementControls(element, type, actions) {
    ...
    controls.querySelector('.element-btn-move-up').addEventListener('click', (e) => {
      e.stopPropagation();
      this.saveState();
      actions.onMoveUp();
      this.refreshElementControls();
    ...
    controls.querySelector('.element-btn-move-down').addEventListener('click', (e) => {
      e.stopPropagation();
      this.saveState();
      actions.onMoveDown();
      this.refreshElementControls();
  ...
  restorePanelState(panel) {
    try {
      const isCollapsed = localStorage.getItem('settings-panel-collapsed') === 'true';
      if (isCollapsed) {
        this.togglePanelCollapse(panel);
      }
    } catch (error) {
  ...
  getColorsFromEditor(modal) {
    return {
      primary: modal.querySelector('.color-picker[data-property="primary"]').value,
      secondary: modal.querySelector('.color-picker[data-property="secondary"]').value,
      accent: modal.querySelector('.color-picker[data-property="accent"]').value,
      background: modal.querySelector('.color-picker[data-property="background"]').value,
  ...
  applyCardGradients(colors) {
    ...
    const cardTypes = [
      { selector: '.testimonial-card', gradient: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})` },
      { selector: '.pricing-card', gradient: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})` },
      { selector: '.feature-card', gradient: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})` },
      { selector: '.step-card, .process-card, [class*="step-"]', gradient: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})` },
      { selector: '.faq-item', gradient: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})` }
    ];
    ...
    cardTypes.forEach(({ selector, gradient }) => {
      ...
      cards.forEach((card, index) => {
        ...
        textElements.forEach(el => {
          if (!el.closest('button')) { // Don't override button text colors
    ...
    const root = document.documentElement;
    root.style.setProperty('--card-gradient-testimonial', `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})`);
    root.style.setProperty('--card-gradient-pricing', `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})`);
    root.style.setProperty('--card-gradient-premium', `linear-gradient(135deg, ${colors.secondary}, ${colors.accent})`);
    root.style.setProperty('--card-gradient-feature', `linear-gradient(135deg, ${colors.primary}, ${colors.secondary})`);
  }
  ...
  async handleImageUpload(file) {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showNotification('Image uploaded and hosted successfully!', 'success');
      } else {
  ...
  handleImageUploadFallback(file) {
    ...
    reader.onload = (e) => {
      ...
      this.updateImageSource(imageUrl, `local-${Date.now()}.${file.name.split('.').pop()}`);
      this.showNotification('Image stored locally (will work in this session)', 'success');
    };
  ...
  getCurrentImageSource(element, type) {
    if (type === 'background') {
      ...
      const img = element.querySelector('img[data-editable="true"], .story-image img');
  ...
  getCurrentFunnelName() {
    ...
    return heroTitle ? heroTitle.textContent.substring(0, 50) + '...' : 'Untitled Funnel';
  }
