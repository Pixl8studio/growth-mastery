...
CREATE POLICY "Users can view own support interactions" ON public.support_interactions FOR
SELECT
  USING (auth.uid () = user_id);

CREATE POLICY "Users can create own support interactions" ON public.support_interactions FOR INSERT
WITH
  CHECK (auth.uid () = user_id);
