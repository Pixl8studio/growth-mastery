...
CREATE POLICY "Users can view own brand designs" ON public.brand_designs FOR
SELECT
  USING (auth.uid () = user_id);

-- Users can create their own brand designs
CREATE POLICY "Users can create own brand designs" ON public.brand_designs FOR INSERT
WITH
  CHECK (auth.uid () = user_id);

-- Users can update their own brand designs
CREATE POLICY "Users can update own brand designs" ON public.brand_designs
FOR UPDATE
  USING (auth.uid () = user_id)
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Users can delete own brand designs" ON public.brand_designs FOR DELETE USING (auth.uid () = user_id);

-- ===========================================
-- TRIGGER: Auto-update timestamps
-- ===========================================
CREATE TRIGGER update_brand_designs_updated_at BEFORE
UPDATE ON public.brand_designs FOR EACH ROW
