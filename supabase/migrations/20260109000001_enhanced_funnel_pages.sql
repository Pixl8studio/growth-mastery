...
-- To reverse this migration during a low-traffic window, run:
--
-- BEGIN;
--
-- -- Drop new tables (CASCADE will drop RLS policies)
-- DROP TABLE IF EXISTS public.order_items CASCADE;
-- DROP TABLE IF EXISTS public.orders CASCADE;
-- DROP TABLE IF EXISTS public.stripe_connect_accounts CASCADE;
-- DROP TABLE IF EXISTS public.thank_you_pages CASCADE;
-- DROP TABLE IF EXISTS public.upsell_pages CASCADE;
-- DROP TABLE IF EXISTS public.checkout_pages CASCADE;
-- DROP TABLE IF EXISTS public.call_bookings CASCADE;
-- DROP TABLE IF EXISTS public.call_booking_pages CASCADE;
-- DROP TABLE IF EXISTS public.confirmation_pages CASCADE;
--
-- -- Restore original ai_editor_pages constraint
...
--
-- DEPLOYMENT NOTES:
-- - This migration adds constraints which require ACCESS EXCLUSIVE locks
-- - Run during low-traffic periods (< 1000 RPM recommended)
-- - Expected execution time: < 5 seconds
-- - Constraint validation on ai_editor_pages happens immediately after DROP
--
-- ===========================================
BEGIN;
...
-- ===========================================
ALTER TABLE public.ai_editor_pages
DROP CONSTRAINT IF EXISTS ai_editor_pages_page_type_check;

ALTER TABLE public.ai_editor_pages
ADD CONSTRAINT ai_editor_pages_page_type_check CHECK (
  page_type IN (
    'registration',
    'watch',
    'enrollment',
...
CREATE INDEX idx_confirmation_pages_vanity ON public.confirmation_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;

CREATE UNIQUE INDEX idx_confirmation_pages_vanity_unique ON public.confirmation_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;
...
CREATE INDEX idx_call_booking_pages_vanity ON public.call_booking_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;

CREATE UNIQUE INDEX idx_call_booking_pages_vanity_unique ON public.call_booking_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS public.checkout_pages (
  ...
  -- Note: order_bump_offer_id is a proper FK for referential integrity
  order_bump_offer_id UUID REFERENCES public.offers (id) ON DELETE SET NULL,
  order_bump_config JSONB DEFAULT '{}', -- { enabled, headline, description, price_override }
...
CREATE INDEX idx_checkout_pages_bump_offer ON public.checkout_pages (order_bump_offer_id)
WHERE
  order_bump_offer_id IS NOT NULL;

CREATE INDEX idx_checkout_pages_vanity ON public.checkout_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;

CREATE UNIQUE INDEX idx_checkout_pages_vanity_unique ON public.checkout_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;
...
CREATE INDEX idx_upsell_pages_vanity ON public.upsell_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;

CREATE UNIQUE INDEX idx_upsell_pages_vanity_unique ON public.upsell_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;
...
CREATE INDEX idx_thank_you_pages_vanity ON public.thank_you_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;

CREATE UNIQUE INDEX idx_thank_you_pages_vanity_unique ON public.thank_you_pages (user_id, vanity_slug)
WHERE
  vanity_slug IS NOT NULL;
...
-- ===========================================
CREATE TRIGGER set_confirmation_pages_updated_at BEFORE
UPDATE ON public.confirmation_pages FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();

CREATE TRIGGER set_call_booking_pages_updated_at BEFORE
UPDATE ON public.call_booking_pages FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();

CREATE TRIGGER set_call_bookings_updated_at BEFORE
UPDATE ON public.call_bookings FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();

CREATE TRIGGER set_checkout_pages_updated_at BEFORE
UPDATE ON public.checkout_pages FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();

CREATE TRIGGER set_upsell_pages_updated_at BEFORE
UPDATE ON public.upsell_pages FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();

CREATE TRIGGER set_thank_you_pages_updated_at BEFORE
UPDATE ON public.thank_you_pages FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();
...
CREATE TRIGGER set_stripe_connect_accounts_updated_at BEFORE
UPDATE ON public.stripe_connect_accounts FOR EACH ROW
EXECUTE FUNCTION public.handle_updated_at ();
...
CREATE POLICY "Users can view their own confirmation pages" ON public.confirmation_pages FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create their own confirmation pages" ON public.confirmation_pages FOR INSERT TO authenticated
WITH
...
CREATE POLICY "Users can delete their own confirmation pages" ON public.confirmation_pages FOR DELETE TO authenticated USING (auth.uid () = user_id);

-- Call Booking Pages Policies
CREATE POLICY "Users can view their own call booking pages" ON public.call_booking_pages FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create their own call booking pages" ON public.call_booking_pages FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Users can delete their own call booking pages" ON public.call_booking_pages FOR DELETE TO authenticated USING (auth.uid () = user_id);

-- Call Bookings Policies
CREATE POLICY "Users can view their own call bookings" ON public.call_bookings FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create call bookings for their pages" ON public.call_bookings FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Users can delete their own call bookings" ON public.call_bookings FOR DELETE TO authenticated USING (auth.uid () = user_id);

-- Allow anonymous booking creation for prospects
CREATE POLICY "Anyone can create call bookings" ON public.call_bookings FOR INSERT TO anon
WITH
  CHECK (true);

-- Checkout Pages Policies
CREATE POLICY "Users can view their own checkout pages" ON public.checkout_pages FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create their own checkout pages" ON public.checkout_pages FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Users can delete their own checkout pages" ON public.checkout_pages FOR DELETE TO authenticated USING (auth.uid () = user_id);

-- Upsell Pages Policies
CREATE POLICY "Users can view their own upsell pages" ON public.upsell_pages FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create their own upsell pages" ON public.upsell_pages FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Users can delete their own upsell pages" ON public.upsell_pages FOR DELETE TO authenticated USING (auth.uid () = user_id);

-- Thank You Pages Policies
CREATE POLICY "Users can view their own thank you pages" ON public.thank_you_pages FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create their own thank you pages" ON public.thank_you_pages FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Users can delete their own thank you pages" ON public.thank_you_pages FOR DELETE TO authenticated USING (auth.uid () = user_id);
...
CREATE POLICY "Users can create orders for their funnels" ON public.orders FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Anyone can create orders" ON public.orders FOR INSERT TO anon
WITH
  CHECK (true);

-- Order Items Policies (access through order ownership)
CREATE POLICY "Users can view items for their orders" ON public.order_items FOR
SELECT
...
CREATE POLICY "Users can create items for their orders" ON public.order_items FOR INSERT TO authenticated
WITH
  CHECK (
    EXISTS (
      SELECT
...
CREATE POLICY "Anyone can create order items" ON public.order_items FOR INSERT TO anon
WITH
  CHECK (true);

-- Stripe Connect Accounts Policies
CREATE POLICY "Users can view their own Stripe account" ON public.stripe_connect_accounts FOR
SELECT
  TO authenticated USING (auth.uid () = user_id);

CREATE POLICY "Users can create their own Stripe account" ON public.stripe_connect_accounts FOR INSERT TO authenticated
WITH
  CHECK (auth.uid () = user_id);
